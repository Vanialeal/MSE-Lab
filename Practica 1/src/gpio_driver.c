/**
 * @file    gpio_driver.c
 * @brief   Implementacion del driver GPIO con acceso directo a registros STM32F4
 *
...
{
    // Verificacion basica de parametros con CMSIS
...
{
    /* Habilitar relojes AHB1 para todos los puertos GPIO */
    ...
    /* Poner todos los pines en modo analogico (reset) */
...
gpio_status_t gpio_setPinMode(GPIO_TypeDef *port, gpio_pin_t pin, gpio_mode_t mode)
{
    ...
    /* Limpiar los 2 bits del modo y escribir el nuevo valor */
...
{
    if (!is_valid(port, pin)) return GPIO_INVALID;
    /* Usar BSRR para escritura atomica en alto */
    port->BSRR = (1U << pin);
    return GPIO_OK;
}
...
{
    if (!is_valid(port, pin)) return GPIO_INVALID;
    /* Usar la mitad alta de BSRR para reset atomico */
...
{
    if (!is_valid(port, pin)) return GPIO_INVALID;
    /* XOR sobre ODR invierte el estado actual del pin */
    port->ODR ^= (1U << pin);
    return GPIO_OK;
}
...
gpio_status_t gpio_readPin(GPIO_TypeDef *port, gpio_pin_t pin, gpio_pin_state_t *state)
{
    ...
    /* Leer el registro de entrada IDR */
    if (port->IDR & (1U << pin)) {
        *state = GPIO_PIN_HIGH;
    } else {
        *state = GPIO_PIN_LOW;
...
gpio_status_t gpio_setAlternateFunction(GPIO_TypeDef *port, gpio_pin_t pin, uint8_t alt_func)
{
    ...
    /* Configurar el pin en modo funcion alternativa */
    port->MODER &= ~(0x3U << (pin * 2U));
    port->MODER |=  (0x2U << (pin * 2U));

    /* Escribir en AFR[0] para pines 0-7, AFR[1] para pines 8-15 */
    if (pin < 8U)
    {
